...
/**
 * Audit Mirror Repo settings.
 *
 * What this is
 * ------------
 * A one-way *write* target for SIEM-owned detections. Every time
 * PurveX syncs detection rules from a linked SIEM, the service
 * serializes the affected rules as YAML and commits them here — so
 * compliance teams get a full git-native change log for rules PurveX
 * doesn't own.
 *
 * How it differs from Detection Sources
 * -------------------------------------
 * * Detection Sources: PurveX *reads* detection YAML from git and
 *   creates/updates Detections in the product.
 * * Audit Mirrors: PurveX *writes* SIEM-synced detection YAML to git
 *   for audit. Never reads back.
 *
 * UX shape
 * --------
 * * Primary: list of configured mirrors with last-publish telemetry.
 * * Secondary: "Add mirror" dialog to register a new repo.
 * * Per-mirror: link one or more SIEM connections so sync auto-pushes,
 *   and a "Bootstrap from SIEM" action for the first-time full export.
 */
export default function DetectionMirrorsPage() {
