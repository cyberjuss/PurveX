...
const RESULT_CONFIG: Record<ResultKey, { icon: typeof CheckCircle2; color: string; dot: string; badge: string }> = {
  PASS: {
    icon: CheckCircle2,
...
export default function TestsHistoryPage() {
  ...
  if (loading) {
    return (
      <PageContainer>
        <Card><CardContent className="pt-6 text-sm text-slate-600 dark:text-slate-300">Loading validation operations...</CardContent></Card>
      </PageContainer>
    );
  }
    ...
    <PageContainer maxWidth="full" className="space-y-5">
      ...
      <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
        ...
        <div className="flex items-center gap-2">
          <MoreMenu />
          <Link href="/run-test">
            <Button size="sm" className="h-9 bg-black text-white hover:bg-slate-800 dark:bg-white dark:text-black dark:hover:bg-slate-200">
      ...
      <div className="flex flex-wrap items-center gap-2">
        <StatusPill label="Needs action" count={statusCounts.attention} color="rose" />
        <StatusPill label="Telemetry blocked" count={statusCounts.telemetry} color="amber" />
        <StatusPill label="In flight" count={statusCounts.inFlight} color="sky" />
        <StatusPill label="Trusted" count={statusCounts.trusted} color="emerald" />
        <span className="text-xs text-slate-400 dark:text-slate-500 ml-1">{tests.length} total runs</span>
        <span className="rounded-full border border-slate-200 bg-slate-50 px-2.5 py-0.5 text-[11px] text-slate-500 dark:border-white/10 dark:bg-white/5 dark:text-slate-400 ml-auto" title="Runs older than the configured retention period are automatically purged. Configure in Settings > Testing Policy.">
          Auto-purge active
        </span>
      </div>
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0 pt-1">
                      <div className="flex flex-wrap items-center gap-2">
                        ...
                        <Badge className={cn("text-[11px]", config.badge)}>{getIssueLabel(test)}</Badge>
                      </div>
            ...
            <div className="divide-y divide-[var(--stroke-soft)]">
              ...
              {filteredTests.map((test) => {
                ...
                return (
                  ...
                  >
                    <Badge className={cn("justify-center text-[11px]", config.badge)}>{getIssueLabel(test)}</Badge>
...
function StatusPill({ label, count, color }: { label: string; count: number; color: "rose" | "amber" | "sky" | "emerald" }) {
  const styles = {
