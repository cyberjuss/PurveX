...
// Mirrors backend/app/security.py:validate_password_complexity exactly.
...
function AcceptInviteContent() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    e.preventDefault();
    if (!token) {
      setError("This invite link is invalid or missing a token. Ask an admin to resend it.");
      return;
    }
    ...
    try {
      const res = await fetchWithApiFallback("/auth/invite/accept", {
        method: "POST",
      ...
      if (!res.ok) {
        ...
        throw new Error(data?.detail || `Activation failed (${res.status})`);
      }
      setPhase("done");
    } catch (err: unknown) {
      setError(err instanceof Error ? err.message : "Unable to activate your account. The link may have expired.");
      setPhase("idle");
    }
  }
        ...
        <div className="w-full max-w-[440px]">
          {phase === "done" ? (
            ...
            <div className="text-center">
              ...
              <p className="mt-2 text-base text-muted-foreground">
                This link is missing its token. Ask an admin to send a new invite.
              </p>
            </div>
          ) : (
            <>
              <div className="mb-10 flex flex-col items-center text-center">
                ...
                <h1 className="text-3xl font-semibold tracking-tight">Welcome to PurveX</h1>
                <p className="mt-2 text-base text-muted-foreground">
                  Choose a password to activate your account. At least 8 characters, with upper and lower case, a number, and a special character.
                </p>
              </div>
