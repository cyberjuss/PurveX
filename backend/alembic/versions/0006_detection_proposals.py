...
def upgrade() -> None:
    ...
    if "detection_proposals" not in sa.inspect(bind).get_table_names():
        op.create_table(
            "detection_proposals",
    ...
    existing_ix = {ix["name"] for ix in sa.inspect(bind).get_indexes("detection_proposals")}
    if "ix_proposals_org_status_created" not in existing_ix:
        op.create_index(
            "ix_proposals_org_status_created",
            "detection_proposals",
            ["organization_id", "status", "created_at"],
        )
    if "ix_proposals_detection_status" not in existing_ix:
        op.create_index(
            "ix_proposals_detection_status",
            "detection_proposals",
            ["detection_id", "status"],
        )
    if "ix_detection_proposals_organization_id" not in existing_ix:
        op.create_index(
